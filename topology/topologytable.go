...
func GenerateTopologyOverviewTable(topologyJSON string, theMap map[uint32]uint32) string {
	...
	htmlOut := `<table border="0" cellpadding="1", cellspacing="1">`
...
func GenerateTopologyOverviewTableIn(typeDef *TopologyHWcTypeDef, in string) string {
	if in == "" {
		return "-"
	}
...
func GenerateTopologyOverviewTableOut(typeDef *TopologyHWcTypeDef, out string) string {
	if out == "" {
		return "-"
	}
...
func GenerateTopologyOverviewTableExt(typeDef *TopologyHWcTypeDef, ext string) string {
	if ext == "" {
		return ""
	}
...
func GenerateTopologyOverviewTableSubIdx(typeDef *TopologyHWcTypeDef, Subidx int) string {
	if Subidx < 0 {
...
func GenerateTopologyOverviewTableDisplay(Disp *TopologyHWcTypeDef_Display) string {
	if Disp == nil {
		return ""
	}
...
func GenerateTopologyOverviewTableTypeOverride(TypeOverride *TopologyHWcTypeDef) string {
	if TypeOverride == nil {
		return ""
	}
